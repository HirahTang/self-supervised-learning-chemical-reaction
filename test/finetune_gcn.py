...
def train(args, model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if args.adv:
            embed_init = model.gnn.x_embedding1(batch.x[:, 0]) + model.gnn.x_embedding2(batch.x[:, 1]) # [num_node, dim_embed]
            ...
            for _ in range(args.adv_step):
                ...
                ## loss for nodes
                pred = model(x_embed, batch.edge_index, batch.edge_attr, batch.batch, args.adv)
                ...
                embed_init = model.gnn.x_embedding1(batch.x[:, 0]) + model.gnn.x_embedding2(batch.x[:, 1])
...
def main():
    ...
    root_dataset = '/share/project/tanghan/sharefs-hantang/chem/dataset'
    #set up dataset
    ...
    if not args.filename == "":
        fname = f'{args.filename}/runs/finetune_cls_runseed{str(args.runseed)}-dset_{args.dataset}'
